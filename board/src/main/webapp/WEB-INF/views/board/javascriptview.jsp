                ...
                <div class="col-sm-12 sm-margin-b-2" style="margin-bottom: 20px;">
                    <div class="wow fadeInLeft" data-wow-duration=".3" data-wow-delay=".3s">
                        <div class="my_box" data-height="height">
                            <form action="${pageContext.request.contextPath }/pages/jslist.js" method="post">
                                <div class="col-md-6">
	                            ...
	                            <div class="mb-5">
	                                <c:if test="${empty sessionScope.session}">
										...
										<c:if test="${sessionScope.session_id.user_id eq board.writer}">
	    		                            <a href="javascript:deleteJs( ${board.post_id })" class="btn btn-danger mt-4" id="delete" type="submit">글 삭제</a>
	    		                       		<a href="${pageContext.request.contextPath }/pages/jsEditChange.js?post_id=${board.post_id }" class="btn btn-primary mt-4" id="list" type="submit">글 수정</a>                          
										</c:if>
...
<%-- 									</c:if> --%>
           					...
           					<!-- 댓글 작성 -->
							<form name="replyForm" action="${pageContext.request.contextPath }/pages/jsReplyWriteOKAction.js" method="post">
								<input type="hidden" name="post_id" value="${board.post_id }">
								...
								<nav aria-label="Page navigation" class="text-center">
									<ul class="pagination">
										<c:if test="${nowPage > 1 }">
											<li class="page-item">							
												<a class="page-link" href="${pageContext.request.contextPath }/pages/jsBoardView.js?page=${nowPage - 1 }&post_id=${board.post_id }">&lt;</a>
											</li>
										</c:if>
										<c:forEach var="i" begin="${startPage}" end="${endPage }">
											<c:choose>
												...
												<c:otherwise>
													<li class="page-item">									
														<a class="page-link" href="${pageContext.request.contextPath }/pages/jsBoardView.js?page=${i }&post_id=${board.post_id }">${i }</a>
													</li>
												</c:otherwise>
											</c:choose>
										</c:forEach>	
...
// 	}
	...
	// [글 삭제] function
	function deleteJs(post_id){
		if (true){
			document.replyForm.action = "${pageContext.request.contextPath}/pages/jsDelete.js?post_id="+post_id;
	...
	function updateReply( reply_id ){
		if (true){
			document.replyForm.action = "${pageContext.request.contextPath}/pages/jsUpdateReply.js?reply_id="+reply_id;
			document.replyForm.submit();
		}
	}
	...
	function deleteReply( reply_id ){
		if (true){
			document.replyForm.action = "${pageContext.request.contextPath}/pages/jsDeleteReply.js?reply_id="+reply_id;
			document.replyForm.submit();
		}
	}
